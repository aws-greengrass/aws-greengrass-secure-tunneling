...
{
  description = "AWS Greengrass Secure Tunneling Component";
    ...
    ({ lib, config, ... }:
      ...
      in
      {
        ...
        checks =
          ...
          in
          {
            build-clang = pkgs: pkgs.secure-tunnel.override
              { stdenv = llvmStdenv pkgs; };

            build-musl-pi = pkgs: pkgs.pkgsCross.muslpi.secure-tunnel;
